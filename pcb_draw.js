...
function padCoreOnly(w, h, color, px = -w/2, py = -h/2) {
  ...
  // Outline when requested (e.g., PCBg without photo)
  if (outlinesVisible) {
    ctx.save();
    ctx.globalCompositeOperation = 'source-over';
    ctx.globalAlpha = 1;
    ctx.shadowBlur  = 0;
    ctx.lineWidth   = Math.max(1, Math.floor(sc(1.2)));
  ...
  // Optional gamma for a snappier fade: uncomment and tweak
  ...
  // Brightness comes from alpha
  ctx.fillStyle = 'rgb(' + color[0] + ',' + color[1] + ',' + color[2] + ')';
  ctx.fillRect(px, py, w, h);
}
  // Draw both passes for a single pad, centered by default
  function renderPadRect(w, h, color, px = -w/2, py = -h/2) {
    padHazeOnly(w, h, color, px, py);
    padCoreOnly(w, h, color, px, py);
  }

  // Adjustment convenience: place + draw a pad at (angle, radius) with size, color, label
  ...
  if (previewType.value === 'PCBb') {
    ...
    var walk = -1;                    // CCW
    var extraRotDeg = 159;            // Alignment tweak
  ...
  // SCW MTRX 64 Pixel rectangular PCB, serpentine (top-left start), 4 rows × 16 cols
  ...
  // SCW MTRX 69 Pixel round PCB (centered row counts: 5,7,9,9,9,9,9,7,5; serpentine order)
  if (previewType.value === 'PCBf') {
    ...
    // Equalized: all haze then all cores - keep consistent brightness
    renderPadsEqualized(items);
    return;
  }
    // ------------------------------------------------------------
  ...
  const photoByCount = {
    11: FIND('pcbg11_image'),
    16: FIND('pcbg16_image'),
    20: FIND('pcbg20_image')
  };
  const photoTuning = {
    11: { scale: 1.18, dx: 0,  dy: 0 },
    16: { scale: 1.23, dx: 0,  dy: 0 },
    20: { scale: 1.28, dx: 0, dy: 0 }
  };

  const img = photoByCount[PCBgLength];
  if (img && img.complete) {
    const t = photoTuning[PCBgLength] || { scale: 1.0, dx: 0, dy: 0 };
    const size = radius * 2 * t.scale;
    const x = centerX - size / 2 + (t.dx || 0);
    const y = centerY - size / 2 + (t.dy || 0);
    ctx.drawImage(img, x, y, size, size);
  }
  outlinesVisible = !img;
